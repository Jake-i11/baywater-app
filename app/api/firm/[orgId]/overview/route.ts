...
import { buildCoachOverview } from "@/lib/firm/reads";
...
export async function GET(
  ...
  try {
    ...
    const body = await buildCoachOverview(
      supabase,
      ctx.context.active.organization_id,
      ctx.context.active.organization_name
    );
    return firmNoStoreJson(body);
  } catch (e) {
    console.error("firm overview error:", e);
    return firmNoStoreJson({ error: "Failed to load overview" }, { status: 500 });
  }
}
